...
<cfset sale_products = product_service.getSaleProducts(limit=url.limit, offset=url.offset)/>
<!--- get the pagination based on the results --->
<cfset utils = new root.org.benton.Utils()/>
<cfset pagination = utils.getPagination(limit=url.limit, offset=url.offset, total=sale_products.total)/>
<cfoutput>
